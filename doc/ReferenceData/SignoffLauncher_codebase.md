...
--- launcher/signoff_applications.yaml ---

# signoff_applications.yaml
applications:
  - name: "Driver_Size_Check"
    runscript_path: "RUNSCRIPTS\\DSC"
    classification:
      design_phase: ["Prelayout", "Postlayout"]
      tool_category: ["SPF"]

  - name: "Level_Shifter_Check"
    runscript_path: "RUNSCRIPTS\\LS"
    classification:
      design_phase: ["Prelayout", "Postlayout"]
      tool_category: ["SPF"]
...
classification_schemes:
  - name: "Design Phase"
    ...
    categories:
      - name: "Prerequesite"
        applications: ["Voltage_Finder", "BA_Dump"]
      - name: "Prelayout"
        description: "R00 ~ R20"
        applications: ["Driver_Size_Check", "Level_Shifter_Check", "Latch_Strenght_Check", "Power_Error_Check", "PN_Ratio", "Fanout_Check", "Driver_Keeper"]
      - name: "Postlayout"
        description: "R30 ~"
        applications: ["Driver_Size_Check", "Power_Error_Check", "Level_Shifter_Check", "Latch_Strenght_Check", "Cana_TR", "PN_Ratio", "Fanout_Check", "Driver_Keeper", "DC_Path", "Float_Node", "Dynamic_DC_Path"]
  ...
  - name: "Tool Category"
    ...
    categories:
      - name: "Prerequesite"
        applications: ["Voltage_Finder", "BA_Dump"]
      - name: "CKT"
        applications: ["Float_Node", "Power_Error_Check", "DC_Path", "PN_Ratio", "Driver_Keeper", "Verilog", "HIZ", "Xtracer"]
      - name: "VTS"
        applications: ["Glitch_Margin_Check_hbm", "Dynamic_DC_Path", "ADV_Mangin_Analyzer"]
      - name: "SPF"
        applications: ["PT_Signoff", "Driver_Size_Check", "Level_Shifter_Check", "Latch_Strenght_Check", "Cana_TR", "Dynamic_Coupling_hbm"]


--- launcher/assets/dashAgGridComponentFunctions.js ---
...
# if os.access(".", os.W_OK) and (not os.path.exists(USER_DIR) or os.stat(USER_DIR).st_uid == os.getuid()):
if os.access(".", os.W_OK):
    DISK_CACHE = os.path.join(USER_DIR, f"cache_{USERNAME}")
    DEFAULT_WORKSPACE_DIR = os.path.join(USER_DIR, "workspace")
else:
    DISK_CACHE = os.path.join(tempfile.gettempdir(), f"{BASE_DIR}_{USERNAME}")
    DEFAULT_WORKSPACE_DIR = os.path.join(tempfile.gettempdir(), "workspace")

os.makedirs(DISK_CACHE, exist_ok=True)
SETTINGS_FILE = os.path.join(DISK_CACHE, "settings")
SIGNOFF_APPLICATION_YAML = os.path.join(DISK_CACHE, "signoff_applications.yaml")

# if not os.path.exists(SIGNOFF_APPLICATION_YAML):
#    shutil.copy2("/user/signoff.dev/launcher/signoff_applications.yaml", SIGNOFF_APPLICATION_YAML)
shutil.copy2("signoff_applications.yaml", SIGNOFF_APPLICATION_YAML)
RUN_SCRIPTS = "/user/signoff.dev/launcher/RUNSCRIPTS"


--- launcher/utils/devworks_api.py ---



--- launcher/utils/config_loader.py ---
...
def set_workspace_dir(path):
    ...
    try:
        ...
        with open(SETTINGS_FILE, "w") as f:
            json.dump({"WORKSPACE_DIR": path}, f)
...
def get_job_count():
    ...
    try:
        ...
        job_count = sum(
            ...
            if os.path.isdir(os.path.join(workspace_dir, item)) and os.path.exists(os.path.join(workspace_dir, item, "job_config.yaml"))
        )
                                ...
                                dmc.Group(
                                    [
                                        ...
                                        dbpc.Tooltip(
                                            ...
                                            children=dbpc.Button(icon="help", small=True),
                                        ),
                                    ],
                        ...
                        dmc.Group([
                            ...
                            dmc.Group([
                                ...
                                dmc.Badge(
                                    ...
                                    variant="filled",
                                    size="sm",
                                    children=os.getenv("USER", "unknown"),
                                    style={"display": "none"}
                                ),
                            ], gap="xs"),
                        ], justify="evenly"),
                    ...
                    children=[
                        ...
                        dmc.StepperStep(
                            ...
                            children=dmc.Stack([
                                ...
                                dmc.Text(
                                    "이 경로가 없는 경우 자동으로 생성됩니다. 경로에 대한 적절한 권한이 있는지 확인하세요.",
                                    mt="md",
                                    size="sm",
                                    c="dimmed"
                                ),
        ...
        def check_storage_permission(storage_path):
            if not storage_path:
                return (
                    ...
                    {"storage": "", "library": "", "cell": "", "user": os.getenv("USER", "unknown")},
                    True
                )            
            # 경로 구성
        ...
        def create_new_library(n_clicks, new_library, storage_path, current_libraries):
            ...
            try:
                ...
                if os.path.exists(new_library_path):
                    return (
                        current_libraries, new_library, "",
                        True, True, True, False, False,
                        f"{new_library_path} 경로가 이미 존재합니다.",
                        "info-sign", "blue", {"display": "block"}
                    )
                    
                # 디렉토리 생성 권한 확인
                if not os.access(signoff_path, os.W_OK):
                    return (
                        current_libraries, None, "",
                        True, True, True, False, False,
                        f"{signoff_path} 경로에 쓰기 권한이 없어 Library를 생성할 수 없습니다.",
                        "warning-sign", "red", {"display": "block"}
                    )
                ...
                return (
                    updated_libraries, new_library, "",
                    True, True, True, False, False,
                    f"{new_library_path} 경로가 생성되었습니다.",
                    "tick-circle", "green", {"display": "block"}
                )
            except Exception as e:
                return (
                    current_libraries, None, "",
                    True, True, True, False, False,
                    f"Library 생성 중 오류 발생: {str(e)}",
                    "error", "red", {"display": "block"}
                )
        ...
        def create_new_cell(n_clicks, new_cell, storage_path, library, current_cells):
            ...
            try:
                ...
                if os.path.exists(new_cell_path):
                    return (
                        current_cells, new_cell, "",
                        True, True, True, False, False,
                        f"{new_cell_path} 경로가 이미 존재합니다.",
                        "info-sign", "blue", {"display": "block"}
                    )
                ...
                if not os.access(library_path, os.W_OK):
                    return (
                        current_cells, None, "",
                        True, True, True, False, False,
                        f"{library_path} 경로에 쓰기 권한이 없어 Cell을 생성할 수 없습니다.",
                        "warning-sign", "red", {"display": "block"}
                    )
            ...
            try:
                if os.path.exists(runscript_path) and os.path.isdir(runscript_path):
                    ...
                    if runscripts:
                        ...
                        return (
                            "RUNSCRIPTS 폴더가 존재하지만 Signoff Application이 없습니다.",
                            "info-sign",
                            "blue",
                            {"display": "block"}
                        )
                    ...
                    return (
                        "RUNSCRIPTS 폴더가 존재하지 않습니다. 설정 후 첫 실행 시 자동으로 생성됩니다.",
                        "info-sign",
                        "blue",
                        {"display": "block"}
                    )
                ...
                return (
                    f"RUNSCRIPTS 폴더 확인 중 오류 발생: {str(e)}",
                    "warning-sign",
                    "orange",
                    {"display": "block"}
                )
...
--- launcher/utils/preprocessing.py ---
...
def parse_netlist_for_subckt_list(netlist_file):
    with open(netlist_file) as nf:
        netlist_contents = nf.readlines()

    netlist_blk = []
    for line in netlist_contents:
        if re.search(".subckt", line):
            subckt_name = line.split(" ")[1]
            netlist_blk.append({"value": subckt_name, "label": subckt_name})

    return netlist_blk


--- launcher/job_run/jobTable.py ---
    ...
    def layout(self):
        ...
        columnDefs = [
            ...
            },
            {
                ...
                "valueFormatter": {
                    "function": "params.data.job_start_time ? d3.timeFormat('%m/%d %H:%M:%S')(d3.timeParse('%Y-%m-%d %H:%M:%S')(params.data.job_start_time)) : '' "
                },
                "flex": 1,
            },
            {
...
--- launcher/job_run/actions/rerun.py ---
...
--- launcher/job_run/actions/stop.py ---
...
--- launcher/job_run/actions/log.py ---
        ...
        def show_log_viewer(data):
            ...
            try:
                ...
                return (
                    {"display": "block"},
                    log_file_buttons,
                    f"Log: {default_log['display_name']}",
                    content,
                    selected_file_info,
                    log_title
                )
...
--- launcher/job_run/actions/result.py ---
...
--- launcher/job_run/actions/remove.py ---
...
--- launcher/job_run/actions/terminal.py ---
...
--- launcher/job_run/jobFiltering.py ---
...
--- launcher/job_run/jobMonitoring.py ---
...
--- launcher/workspace/LS_SSPHVCT_20250318_01/job_config.yaml ---
...
--- launcher/RUNSCRIPTS/DSC/make_csv.py ---
...
--- launcher/RUNSCRIPTS/DSC/pre_setting.py ---

import os


def write_tech(mp_file, edr_file, process_corner, spc_file, voltage_corner="", temp_corner=""):

    tech_file = open("./SETUP/tech_file", "w")

    # Scale option 설정
    tech_file.write(".option scale=" + os.environ.get("scale", "1") + "\n")

    # Corner 정보 추가
    if voltage_corner and temp_corner:
        tech_file.write("*** corner *** vol_corner: " + voltage_corner + " temp_corner: " + temp_corner + "\n")
    ...
    # RPASS file이 있는 경우
    if os.environ.get("rpass"):
        tech_file.write(".lib\t" + f"'{os.environ['rpass']}'" + "\t")
        if process_corner[0].lower() == "s":
            tech_file.write("s\n")
        elif process_corner[0].lower() == "f":
            tech_file.write("f\n")
        else:
            tech_file.write("t\n")
...
def write_power(voltage_corner):
    ...
    if voltage_corner == "LV":
        vdd_list_file = os.environ["LV_VDD_List"]
        gnd_list_file = os.environ["LV_GND_List"]
    elif voltage_corner == "HV":
        vdd_list_file = os.environ["HV_VDD_List"]
        gnd_list_file = os.environ["HV_GND_List"]
    else:
        return
...
def main():
    ...
    corner = os.environ.get(f"{app_name}_corner")

    # Get input file paths
    mp_file = os.environ["mp"]
    edr_file = os.environ["edr"]
    spc_file = os.environ["spc"]

    # Get corner setup if exists
    process_corner = os.environ.get(f"{app_name}_{corner}_process", "TT")  # Default TT
    voltage_corner = os.environ.get(f"{app_name}_{corner}_voltage", "")
    temp_corner = os.environ.get(f"{app_name}_{corner}_temperature", "")
...
--- launcher/RUNSCRIPTS/DSC/input_config.yaml ---

Driver_Size_Check: 
  color: "#4263eb"
  description: "Driver Size Check - Verifies transistor sizing for adequate drive strength"
  inputs:
    - name: SPC_Netlist_File
      type: file_input
      required: true 
    ...
    - name: Rpass_File
      type: file_input
      required: false
      default: "/user/dramrez/DRAMREZ/MODEL/D158/MOB/D1z_XR_V1.07.99A_230926/MP/RPASS.lib"
  ...
  pvt_inputs:
    ...
    - name: LV_VDD_List_File
      type: file_input
      required: true
      default: "/user/verifier11/jenny/TEST_LIB/LV_VDD_List.inc"
    ...
    - name: LV_GND_List_File  
      type: file_input
      required: true
      default: "/user/verifier11/jenny/TEST_LIB/LV_GND_List.inc"
    ...
    - name: HV_VDD_List_File
      type: file_input
      required: true
      default: "/user/verifier11/jenny/TEST_LIB/HV_VDD_List.inc"
    ...
    - name: HV_GND_List_File
      type: file_input
      required: true
      default: "/user/verifier11/jenny/TEST_LIB/HV_GND_List.inc"
      description: "File containing high voltage GND node list"
      depends_on:
        voltage: "HV"

--- launcher/RUNSCRIPTS/DSC/run.sh ---
...
--- launcher/RUNSCRIPTS/LS/input_config.yaml ---
...
--- launcher/job_set/page.py ---

import dash_mantine_components as dmc
from dash import Input, Output, State, ALL, ctx, exceptions, no_update
from utils.logger import logger
...
class SetPage:
    ...
    def __init__(self, app):
        self.logger = logger.getChild("SetPage")
    ...
    def _collect_input_values(self, file_inputs, number_inputs, text_inputs, select_inputs, multiselect_inputs):
                ...
                for input_type, required_fields in required_inputs.items():
                    for input_name, input_config in required_fields.items():
                        ...
                        if input_name not in input_values[input_type] or not input_values[input_type][input_name]:
                            self.logger.debug(f"Required {input_type} input '{input_name}' is missing")
                            return False
    ...
    def _validate_pvt_settings(self, signoff_apps, PVTs):
        ...
        for i, app in enumerate(signoff_apps):
            ...
            # PVT가 필요한데 입력이 없는 경우
            if not PVTs[i]:
                self.logger.debug(f"PVT required for {app} but no PVT input provided")
                return False
            ...
            for pvt_row in PVTs[i]:
                try:
                    ...
                    if not all(pvt_values.values()):
                        self.logger.debug(f"Incomplete PVT values for {app}: {pvt_values}")
                        return False
                except Exception as e:
                    self.logger.error(f"Error validating PVT row: {str(e)}")
                    return False
    ...
    def register_callbacks(self, app):
        ...
        def update_job_counter_to_add_queue(multiselect_values, pvt_container):
            total_pvt_jobs = 0
            for pvts in pvt_container:
        ...
        @app.callback(
            ...
            Input({"type": "file-input", "index": ALL}, "value"),
            Input({"type": "number-input", "index": ALL}, "value"),
            Input({"type": "text-input", "index": ALL}, "value"),
            Input({"type": "select-input", "index": ALL}, "value"),
            Input({"type": "multiselect-input", "index": ALL}, "value"),
            prevent_initial_call=True,
        )
        ...
        @app.callback(
            ...
            State({"type": "file-input-text", "index": ALL}, "value"),
            State({"type": "number-input", "index": ALL}, "value"),
            State({"type": "text-input", "index": ALL}, "value"),
            State({"type": "select-input", "index": ALL}, "value"),
            State({"type": "multiselect-input", "index": ALL}, "value"),
                    ...
                    if is_pvt_required and pvt_splits[i]:
                        for pvt_row in pvt_splits[i]:
                            try:
                                ...
                                self.logger.error(f"잘못된 PVT 형식: {pvt_row}")
                                continue
                    else:
                    ...
                    for input_config in app_config.get("inputs", []) + app_config.get("pvt_inputs", []):
                        ...
                        if input_name in input_values:
                            ...
                            if input_config["type"] == "number_input" and value:
                                value = float(value)
                            job_inputs[input_config["name"]] = value
                    ...
                    for subckt in subckt_list:
                        ...
                        for corner in corners:
                            try:
                                ...
                                self.logger.error(
                                    f"{app_name} 작업 생성 실패: {str(e)}",
                                    exc_info=True,
                                )
                                continue

                except Exception as e:
                    self.logger.error(f"{app_name} 처리 중 에러 발생: {str(e)}", exc_info=True)
                    continue
        ...
        def run_queue(n_clicks, queue_data):
            if not n_clicks or not queue_data:
            ...
            for job in queue_data:
                try:
                    ...
                    pass
            return [], True, "Created and started Jobs"


--- launcher/job_set/jobManager.py ---
...
class JobManager:
    ...
    def __init__(self, app_name, subckt, job_name, corner, inputs):
    ...
    def _create_directories(self):
        ...
        self.job_dir = os.path.join(get_workspace_dir(), self.job_name)
        os.makedirs(self.job_dir, exist_ok=True)

        # Copy RunScripts
        app_scripts_dir = os.path.join(RUN_SCRIPTS, self.app_name)
    ...
    def _create_env_file(self):
        ...
        with open(self.env_file, "w") as f:
            # Basic settings
            # f.write(f"setenv PATH {os.path.join(RUN_SCRIPTS, 'bin')}:$PATH\n\n")
            ...
            for setting in app_config[self.app_name]["inputs"]:
                ...
                if setting["type"] in ["file_input", "text_input"]:
    ...
    def run(self):
        ...
        try:
            ...
            # Execute job
            with open(os.path.join(self.app_run_dir, "LOG", "stdout.log"), "w") as stdout_file:
                cmd = f"source {self.env_file} && {os.path.join(self.app_run_dir, 'run.sh')}"
                process = subprocess.Popen(
                    cmd,
                    shell=True,
                    executable="/bin/csh",
                    cwd=self.app_run_dir,
                    stdout=stdout_file,
                    stderr=stdout_file,
            ...
            with open(os.path.join(self.job_dir, "job_config.yaml"), "w") as f:
                yaml.dump(self.job_config, f, default_flow_style=False)
...
class JobQueue:
    def __init__(self, app):
        self.queue_backup_file = os.path.join(DISK_CACHE, f"queue_data_{os.getenv('USER', 'unknown')}.json")
        self._register_callbacks(app)

    def parse_job_data(self, application, corner, job_inputs):
    ...
    def extract_inputs(self, job_data):
        ...
        inputs = {key: value for key, value in job_data.items() if key not in exclude_keys and value is not None}
        return inputs
    ...
    def _register_queue_table_action_callbacks(self, app):
        ...
        def delete_selected_rows(n_clicks):
            if not n_clicks:
                raise exceptions.PreventUpdate
            return True
...
class JobCards:
                ...
                [
                    dmc.CardSection(
                        [
                            ...
                            dmc.SegmentedControl(
                                ...
                                fullWidth=True,
                                color="indigo",
                                value=scheme_data[0]["value"] if scheme_data else "none",
                            ),
                            dmc.Space(h=10),
                    ...
                    dmc.CardSection(
                        ...
                        children=[
                            ...
                            html.Div(
                                [self.create_pvt_condition_row(job_id, 0)],
                                id={"type": "pvt-container", "index": job_id},
                            ),
                        ],
                        inheritPadding=True,
                        py="xs",
    ...
    def create_pvt_condition_row(self, job_id, row_id, process="TT", voltage="HV", temperature="CT"):
        return dmc.Group(
            [
                dmc.Select(
                    label="Process",
            ...
            for app_name in category.get("applications", []):
                ...
                if app_name in self.app_configs:
                    category_apps.append({
                        "value": unique_id,  # 고유 식별자
                        "label": app_name,   # 표시용 이름
                        "app_name": app_name # 실제 앱 이름 (추후 처리용)
                    })
    ...
    def _register_callbacks(self, app):
        ...
        def delete_job_card(n_clicks, current_jobs):
            ...
            for t in ctx.triggered:
                idx = json.loads(t["prop_id"].split(".")[0])["index"]
                if idx == deleted_index and t["value"] == 0:
                    raise exceptions.PreventUpdate

            current_jobs = [job for job in current_jobs if job["props"]["id"]["index"] != deleted_index]
            # reset copy-job-card-btn-n_clicks
        ...
        def update_job_card_color(selected_value, existing_corners):
            ...
            if "_" in selected_value:
                ...
                app_name = selected_value.split("_", 1)[1]
            else:
                # 기존 형식 (고유 식별자가 아닌 경우)
            ...
            return pvt_style, card_style, selected_color, existing_corners
...
--- launcher/job_set/jobInputs.py ---

import dash_blueprint_components as dbpc
import dash_mantine_components as dmc
from dash import Input, Output, State, dcc, html, ALL, ctx, no_update, exceptions
...
class JobInputs:
    ...
    def _get_input_field(self, input_config, app_name, preset_value=None):
        """
        ...
        if field_id in self.input_field_registry:
        ...
        # 필드 타입에 따른 컴포넌트 생성
        if field_type == "file_input":
            field = self._file_input_field(input_config["name"], value, [app_name], tooltip)
        elif field_type == "number_input":
            field = self._number_input_field(
                input_config["name"],
                value,
                [app_name],
                tooltip,
                input_config.get("unit", ""),
            )
        elif field_type == "text_input":
            field = self._text_input_field(input_config["name"], value, [app_name], tooltip)

        elif field_type == "select_input":
            field = self._select_input_field(input_config["name"], value, [app_name], tooltip, input_config["datas"])
        elif field_type == "text_input":
            field = self._multiselect_input_field(input_config["name"], value, [app_name], tooltip, input_config["datas"])
    ...
    def _number_input_field(self, name, value, app_names, tooltip, unit=""):
            ...
            children=[
                dbpc.Tooltip(
                    ...
                    children=dmc.NumberInput(
                        id={"type": "number-input", "index": name.replace("_", "-")},
                        value=str(value),
                        size="xs",
                        suffix=unit,
                    ),
    ...
    def _text_input_field(self, name, value, app_names, tooltip):
        return dbpc.FormGroup(
            children=[
                dbpc.Tooltip(
                    content=tooltip,
                    ...
                    children=dbpc.InputGroup(
                        id={"type": "text-input", "index": name.replace("_", "-")},
                        value=value,
                    ),
                )
            ],
    ...
    def _file_input_field(self, name, value, app_names, tooltip):
        return dbpc.FormGroup(
            children=[
                dbpc.Tooltip(
                    content=tooltip,
                    ...
                    children=dmc.TextInput(
                        ...
                        required=True,
                        id={"type": "file-input", "index": name.replace("_", "-")},
                    ),
                )
            ],
    ...
    def _select_input_field(self, name, value, app_names, tooltip, datas):
        return dbpc.FormGroup(
            children=[
                dbpc.Tooltip(
                    content=tooltip,
                    ...
                    minimal=True,
                    children=dmc.Select(
                        id={"type": "select-input", "index": name.replace("_", "-")},
                        value=str(value),
                        data=datas,
                        size="xs",
                    ),
    ...
    def _multiselect_input_field(self, name, value, app_names, tooltip, datas):
        return dbpc.FormGroup(
            children=[
                dbpc.Tooltip(
                    content=tooltip,
    ...
    def _register_callbacks(self, app):
        ...
        def update_input_fields(signoff_values):
            ...
            if optional_file_inputs + optional_number_inputs + optional_text_inputs + optional_text_inputs + optional_multiselect_inputs:
                optional_inputs = dmc.Stack(
                    children=[
                        ...
                        dbpc.Collapse(
                            ...
                            isOpen=False,
                            children=[
                                dmc.SimpleGrid(optional_file_inputs, cols=1, verticalSpacing="xs"),
                                dmc.SimpleGrid(
                                    optional_number_inputs,
                                    cols=4,
                                    verticalSpacing="xs",
        ...
        @app.callback(
            ...
            Input({"type": "select-process", "index": ALL, "row": ALL}, "id"),
            prevent_initial_call=True,
        )
                        ...
                        for input_config in app_config.get("pvt_inputs", []):
                            ...
                            for pvt_type, required_values in depends_on.items():
                                if current_pvt[pvt_type][i] == required_values:
                                    field = self._get_input_field(input_config, app_name)
...
from utils.settings import DISK_CACHE
from utils.workspace import Workspace, get_workspace_dir, get_job_count
from utils.logger import logger
                ...
                dmc.GridCol(
                    dmc.Group(
                        [
                            dbpc.Button(
                                dmc.Indicator(
                                    os.path.basename(get_workspace_dir()),
                                    color="indigo",
...
def create_app():
    ...
    workspace_manager = Workspace(app)
    ...
    def handle_page_navigation(set_clicks, run_clicks, move_runpage, move_setpage):
        """Handle navigation between pages"""
        try:
            trigger = ctx.triggered_id
            if trigger == "set-page-btn":
                return set_page.layout()
            elif trigger == "run-page-btn":
                return run_page.layout()
            elif trigger == "job-created-alert" and move_runpage:
                set_props("job-created-alert", {"isConfirmed": False})
                return run_page.layout()
            elif trigger == "job-rerun-alert" and move_setpage:
                set_props("job-rerun-alert", {"isConfirmed": False, "isOpen": False})
                return run_page.layout()
            return no_update
        except Exception as e:
